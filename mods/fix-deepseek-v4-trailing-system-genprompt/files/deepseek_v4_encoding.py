# __spark_trailing_system_genprompt__
...
Spark fix: trailing-system-message empty output
--------------------------------------------------
When a request's last message has role=system (e.g. Claude Code's
"<total_tokens>N tokens left</total_tokens>" low-context reminder, injected
as the final message of a turn), the stock render_message() generation-prompt
branch below only fires for role in ("user", "developer"), so the rendered
prompt ends on raw system-message text with no "<｜Assistant｜><think>"
suffix at all. The model is handed an unterminated prompt and emits EOS as
its very first token -- a deterministic empty-output bug, not sampling
randomness. Confirmed by direct encode_messages() comparison and by live
replay (~77% empty-output rate on affected traffic shapes, 0% once fixed).
Fix: broaden the generation-prompt condition to fire for any role except
"assistant" (already-terminated turn) and "latest_reminder" (own token/flow),
instead of allow-listing only "user"/"developer". See the marked line below.
"""
...
def render_message(index: int, messages: List[Dict[str, Any]], thinking_mode: str, drop_thinking: bool = True, reasoning_effort: Optional[str] = None) -> str:
    ...
    if task is not None:
        ...
        # __spark_trailing_system_genprompt__: broadened from the stock
        # `in ["user", "developer"]` allow-list. We reach this branch
        # whenever it's genuinely time to hand off to the model (end of
        # the message list, or the next message is assistant/
        # latest_reminder) -- that's true regardless of *this* message's
        # role. The stock allow-list silently dropped the generation-prompt
        # suffix whenever the last message was role=system (or any other
        # non-user/developer role), leaving the model an unterminated
        # prompt that it would resolve by emitting EOS immediately.
