...
-------------------------
Adds two DeepSeek-V4 DSML failure-mode recoveries on top of the stock
engine (see deepseek_v4.py module docstring for the corpus evidence):

* **Provisional tool-call hold** (``_begin_recovery_hold`` /
  ``_advance_recovery_hold`` / ``_abort_recovery_hold``): a transition
  marked ``provisional_tool_call=True`` buffers its events instead of
  emitting them immediately. The buffered tool name is validated by
  ``recovery_tool_name_validator`` (set by the owning parser); if valid
  the  hold is committed at ``commit_provisional_tool_call=True`` and
  the buffered events are released as a real tool call. If invalid, or
  the held name looks nothing like a real tool name (too long /
  contains a newline), the hold aborts and the raw buffered text is
  replayed as ordinary content -- nothing is lost.
* **Orphan closing-tail absorption**: a strong DSML/ASCII closer
  (``INVOKE_END``, ``ASCII_INVOKE_END``, ``TOOL_END``) reaching
  ``_on_terminal`` while ``state == CONTENT`` -- i.e. with no active
  recovery hold and no real invoke content -- can only be a dropped
  tool call. It is absorbed (never leaked as text) and a synthetic
  ``bash`` tool call is emitted once per turn asking the model to
  re-issue its last tool call. A lone ``PARAM_CLOSE``/``ASCII_PARAM_CLOSE``
  in ``CONTENT`` is absorbed silently (no regen) to avoid a spurious
  call on ordinary prose that happens to quote a closing tag.

This 0904 variant retains the upstream ``token_count`` / ``reasoning_token_count``
refactor; the recovery branches are orthogonal to it.
"""
...
# Strong closers: reaching one of these in CONTENT with no active recovery
# hold proves a tool call was dropped (every legitimate wrapper closes while
# still in a tool state, never in CONTENT). Absorbed + triggers a synthetic
# bash regenerate call, once per turn.
_SPARK_ORPHAN_TAIL_CLOSERS = frozenset({"INVOKE_END", "ASCII_INVOKE_END", "TOOL_END"})
# Weak closers: absorbed silently, no regen (avoids a spurious call on
# prose that merely quotes a closing tag with nothing else orphaned).
_SPARK_ORPHAN_TAIL_SILENT = frozenset({"PARAM_CLOSE", "ASCII_PARAM_CLOSE"})

# The synthetic bash command the agent executes when an orphan DSML closing
# tail is detected. Its stdout is the tool result fed back to the model, so
# the text doubles as the prompt asking the model to re-issue the last call.
# Deliberately never mentions "DSML" or any parser-internal term: doing so
# was observed to increase the frequency of this exact failure mode.
_REGEN_TAIL_MESSAGE = (
...
class StreamingParserEngine:
    ...
    def __init__(
        ...
        # transitions. Intentionally survives reset() so the owning
        # ParserEngine can bind a stable callback once.
    ...
    def reset(self, initial_state: ParserState | None = None) -> None:
        ...
        # A recovered invoke started outside a valid outer tool wrapper, so
        # no matching outer closer is coming. Set once a hold commits: the
        # very next stray closer (if any) is absorbed silently rather than
        # triggering a regenerate, since it is the harmless leftover from
        # the recovered call rather than a genuinely dropped one.
    ...
    def _on_terminal(
        ...
        if transition is None:
            ...
            if self._recovery_hold_active and self.state == ParserState.TOOL_NAME:
                # A non-name terminal interrupted the held name (e.g. a
                # newline or another opener) — this is not a real tool
                # name, replay the hold as plain content and reprocess.
            ...
            if self._recovery_hold_active and self.state == ParserState.TOOL_ARGS:
                ...
                # no state transition; during recovery they stay buffered
                # as arguments, same as the non-recovery path.
                return self._emit_for_state(value, token_count)
            if (
                not self._recovery_hold_active
                and not self.skip_tool_parsing
                and self.state == ParserState.CONTENT
            ):
                if terminal in _SPARK_ORPHAN_TAIL_CLOSERS:
                    if self._tail_regen_done or self._tail_content_since_reset:
                        return []
                    self._tail_regen_done = True
                    return self._spark_regenerate_tail()
                if terminal in _SPARK_ORPHAN_TAIL_SILENT:
                    # A DSML parameter-close tag is itself strong evidence
                    # that whatever text just preceded it (typically the
                    # tag's own leaked opener + attribute text, e.g. a bare
                    # parameter fragment with no invoke) was DSML garbage,
                    # not genuine prose. Clear the content flag so a
                    # strong closer immediately following still regens
                    # instead of being permanently suppressed by that
                    # leaked fragment text.
                    self._tail_content_since_reset = False
                    return []
    ...
    def _advance_recovery_hold(
        ...
        if self.state == ParserState.TOOL_ARGS:
            ...
            # Do not leave it in TOOL_BETWEEN: subsequent text is content,
            # and any following bare invoke re-enters the provisional path.
            self.state = ParserState.CONTENT
    ...
    def _hold_recovery_text(self, text: str, token_count: int = 0) -> list[SemanticEvent]:
        ...
        self._recovery_hold_events.extend(
            self._emit_for_state_now(text, token_count)
        )
        return []

    def _emit_for_state_now(self, text: str, token_count: int = 0) -> list[SemanticEvent]:
