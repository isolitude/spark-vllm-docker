...
Spark recovery extension: gives the synthetic ``bash`` regenerate call
emitted by StreamingParserEngine._spark_regenerate_tail (see
streaming_parser_engine.py) its exact JSON arguments verbatim, bypassing
whatever arg_converter the concrete parser installs -- DeepSeek's DSML
converter would otherwise strip the bare ``{"command": ...}`` dict to
``{}``. Real tool calls named "bash" with actual DSML markup arguments are
unaffected and fall through to the normal converter flush.
"""
...
class ParserEngine(Parser):
    ...
    def _check_skip_tool_parsing(
        ...
        if tools:
            self._tools = tools
        self._spark_tail_regen_eligible = bool(
            tools
            and find_tool_name(tools, "bash")
    ...
    def _handle_tool_end(
        ...
        if slot_before_flush.name == "bash":
            spark_args_json = slot_before_flush.args.strip()
            spark_is_synthetic = bool(
                spark_args_json.startswith("{")
                and getattr(self, "_spark_tail_regen_eligible", True)
                and find_tool_name(self._tools, "bash")
            )
            if spark_is_synthetic:
                # Synthetic orphan-tail regenerate: args are already the
                # exact JSON dictated by the streaming engine, emit them
                # verbatim -- the DeepSeek arg converter would otherwise
                # strip bare JSON to {}.
                ...
                return
            elif not getattr(self, "_spark_tail_regen_eligible", True):
                # bash not declared on the request (or tool_choice=none):
                # the residual was already absorbed by the streaming
                # engine; drop the call so it never reaches the generic
                # commit path (where an empty _tools would let
                # _is_valid_tool_name accept any name and emit a
                # spurious {}).
                return
            # else: a REAL bash call with DSML markup args -- fall through
            # to the normal converter flush below so it still produces
            # real JSON.
