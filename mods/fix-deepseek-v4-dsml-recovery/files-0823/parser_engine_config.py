...
class Transition:
    ...
    # Mark this transition as a provisional tool-call recovery path: the
    # engine buffers its events, validates the completed tool name through
    # the parser's validator, and commits only at the invoke-end transition.
    provisional_tool_call: bool = False
    # Commit a buffered provisional call at this transition; recovery paths
    # that leave tool arguments any other way are restored as ordinary text.
