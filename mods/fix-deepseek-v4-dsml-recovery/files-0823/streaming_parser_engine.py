...
* **Provisional tool-call hold** (``_begin_recovery_hold`` /
  ...
  ``recovery_tool_name_validator`` (set by the owning parser); if valid the
  hold is committed at ``commit_provisional_tool_call=True`` and the
  buffered events are released as a real tool call. If invalid, or the
  held name looks nothing like a real tool name (too long / contains a
  newline), the hold aborts and the raw buffered text is replayed as
  ordinary content -- nothing is lost.
* **Orphan closing-tail absorption**: a strong DSML/ASCII closer
  ...
  recovery hold and no real invoke content -- can only be a dropped tool
  call. It is absorbed (never leaked as text) and a synthetic ``bash``
  tool call is emitted once per turn asking the model to re-issue its last
  tool call. A lone ``PARAM_CLOSE``/``ASCII_PARAM_CLOSE`` in ``CONTENT`` is
  absorbed silently (no regen) to avoid a spurious call on ordinary prose
  that happens to quote a closing tag.
"""
